...
module.exports = async (req, res) => {
  ...
  if (!r.length) return res.status(401).json({ success:false, error:'Unauthorized' });
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    if (!publicUrl) {
      ...
      try { fs.copyFileSync(video.filepath || video.file, dest); } catch(e) { try { fs.renameSync(video.filepath, dest); } catch(e2) { console.error(e2); } }
